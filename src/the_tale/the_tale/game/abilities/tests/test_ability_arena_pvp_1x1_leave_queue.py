...
class ArenaPvP1x1LeaveQueueAbilityTest(helpers.UseAbilityTaskMixin, utils_testcase.TestCase):
    ...
    def test_use_waiting_battle_state(self):
        ...
        self.assertEqual(pvp_models.Battle1x1.objects.filter(state=pvp_models.BATTLE_1X1_STATE.WAITING).count(), 1)
        ...
        self.assertEqual(pvp_models.Battle1x1.objects.filter(state=pvp_models.BATTLE_1X1_STATE.WAITING).count(), 0)
