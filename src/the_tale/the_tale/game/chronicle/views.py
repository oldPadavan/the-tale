...
class ChronicleResource(utils_resources.Resource):
    ...
    def index(self, page=None, place=None, person=None):
        ...
        if page is None:
            page = utils_pagination.Paginator.get_page_numbers(records_count, conf.chronicle_settings.RECORDS_ON_PAGE)
            if page == 0: page = 1

        page = int(page) - 1

        paginator = utils_pagination.Paginator(page, records_count, conf.chronicle_settings.RECORDS_ON_PAGE, url_builder, inverse=True)
