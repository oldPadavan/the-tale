...
class IndexRequestsTest(utils_testcase.TestCase):
    ...
    def test_full_page(self):
        texts = []
        for i in range(conf.chronicle_settings.RECORDS_ON_PAGE):
    ...
    def test_second_page(self):
        for i in range(conf.chronicle_settings.RECORDS_ON_PAGE+1):
            self.create_record(i)

        texts = ['record_text_%d_%d' % (0, conf.chronicle_settings.RECORDS_ON_PAGE)]
