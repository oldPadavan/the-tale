...
def update_map(index):
    ...
    generator = prototypes.WorldInfoPrototype.get_by_id(storage.map_info_storage.item.world_id).generator

    generator.clear_power_points()
    generator.clear_biomes()

    if generator.w != conf.map_settings.WIDTH or generator.h != conf.map_settings.HEIGHT:
        dx, dy = generator.resize(conf.map_settings.WIDTH, conf.map_settings.HEIGHT)
    ...
    storage.map_info_storage.set_item(prototypes.MapInfoPrototype.create(turn_number=turn.number(),
                                                      width=generator.w,
                                                      height=generator.h,
                                                      terrain=terrain,
    ...
    data = {'width': generator.w,
            'height': generator.h,
            'map_version': storage.map_info_storage.version,
            'format_version': '0.1',
            ...
            'roads': dict( (road.id, road.map_info() ) for road in roads_storage.roads_storage.all()) }
