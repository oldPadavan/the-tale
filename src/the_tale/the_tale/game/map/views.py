...
def index(context):
    return dext_views.Page('map/index.html',
                           content={'current_map_version': storage.map_info_storage.version,
                                    'resource': context.resource})


@utils_api.Processor(versions=(conf.map_settings.REGION_API_VERSION,))
...
@utils_api.Processor(versions=(conf.map_settings.REGION_VERSIONS_API_VERSION,))
...
def cell_info(context):
    ...
    if x < 0 or y < 0 or x >= conf.map_settings.WIDTH or y >= conf.map_settings.HEIGHT:
        raise dext_views.ViewError(code='outside_map', message='Запрашиваемая зона не принадлежит карте')

    map_info = storage.map_info_storage.item
