...
class Upbringing(dext_forms.Form):
    value = dext_fields.RelationField(label='происхождение', relation=beings_relations.UPBRINGING)
...
class DeathAge(dext_forms.Form):
    value = dext_fields.RelationField(label='возраст смерти', relation=beings_relations.AGE)
...
class DeathType(dext_forms.Form):
    value = dext_fields.RelationField(label='способ первой смерти', relation=beings_relations.FIRST_DEATH)
