...
def to_ability(ability_id):
    return heroes_habilities.ABILITIES[ability_id]
...
class MobRecordBaseForm(dext_forms.Form):
    ...
    type = dext_fields.TypedChoiceField(label='тип', choices=MOB_TYPE_CHOICES, coerce=beings_relations.TYPE.get_from_name)
    ...
    def clean_abilities(self):
        ...
        if battle_abilities.HIT.get_id() not in abilities_ids:
            abilities_ids.append(battle_abilities.HIT.get_id())
