...
class PlaceRenamingTests(helpers.BaseTestPrototypes):
    ...
    def create_place_renaming_bill(self, index):
        self.bill_data = bills.place_renaming.PlaceRenaming(place_id=self.place1.id, name_forms=names.generator().get_test_name('new_name_%d' % index))
    ...
    def test_create(self):
        ...
        self.assertEqual(forum_models.Post.objects.all()[0].markup_method, forum_models.MARKUP_METHOD.POSTMARKUP)
    ...
    def test_apply(self):
        ...
        data = self.bill.user_form_initials
        data.update(linguistics_tests_helpers.get_word_post_data(new_name, prefix='name'))
        data['approved'] = True
        form = self.bill.data.get_moderator_form_update(data)
