...
class BaseTestRequests(utils_testcase.TestCase):
    ...
    def setUp(self):
        ...
        self.account1._model.created_at -= datetime.timedelta(days=conf.bills_settings.MINIMUM_BILL_OWNER_AGE)
        self.account1.save()

        self.account2 = self.accounts_factory.create_account()
        # self.account2.prolong_premium(30)
        self.account2._model.created_at -= datetime.timedelta(days=conf.bills_settings.MINIMUM_BILL_OWNER_AGE)
        self.account2.save()
        ...
        SubCategory.objects.create(caption=conf.bills_settings.FORUM_CATEGORY_UID + '-caption',
                                   uid=conf.bills_settings.FORUM_CATEGORY_UID,
...
class TestIndexRequests(BaseTestRequests):
    ...
    def test_bill_creation_locked_message(self):
        bill_data = bills.place_renaming.PlaceRenaming(place_id=self.place1.id, name_forms=names.generator().get_test_name('new_name_1'))
    ...
    def test_one_page(self):
        ...
        bill_data = bills.place_renaming.PlaceRenaming(place_id=self.place2.id, name_forms=names.generator().get_test_name('new_name_2'))
    ...
    def create_two_pages(self):
        bill_data = bills.place_renaming.PlaceRenaming(place_id=self.place1.id, name_forms=names.generator().get_test_name('new_name_1'))
        self.create_bills(conf.bills_settings.BILLS_ON_PAGE, self.account1, 'Caption-a1-%d', bill_data)
    ...
    def test_filter_by_user(self):
        ...
        account_1_texts = [('pgf-no-bills-message', 0),
                           ...
                           ('Caption-a2-0', 0),
                           ('Caption-a2-2', 0),
                           (self.account1.nick, conf.bills_settings.BILLS_ON_PAGE + 2), #1 for main menu, 1 for filter text
    ...
    def test_filter_by_place(self):
        ...
        bill_data = bills.place_renaming.PlaceRenaming(place_id=self.place2.id, name_forms=names.generator().get_test_name('new_name_1'))
...
class TestShowRequests(BaseTestRequests):
    ...
    def test_is_fast(self):
        bill_data = bills.place_renaming.PlaceRenaming(place_id=self.place1.id, name_forms=names.generator().get_test_name('new_name_2'))
...
class TestCreateRequests(BaseTestRequests):
    ...
    def get_post_data(self, depends_on_id=None):
        ...
        data = linguistics_tests_helpers.get_word_post_data(new_name, prefix='name')
    ...
    @mock.patch('the_tale.game.bills.conf.bills_settings.MINIMUM_BILL_OWNER_AGE', conf.bills_settings.MINIMUM_BILL_OWNER_AGE + 1)
...
class TestModerateRequests(BaseTestRequests):
    ...
    def get_post_data(self):
        ...
        data.update(linguistics_tests_helpers.get_word_post_data(self.bill.data.name_forms, prefix='name'))
        data['approved'] = True
        return data
