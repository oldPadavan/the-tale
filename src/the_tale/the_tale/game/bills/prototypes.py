...
class BillPrototype(utils_prototypes.BasePrototype):
    ...
    def time_before_voting_end(self):
        return max(datetime.timedelta(seconds=0),
                   (self._model.updated_at + datetime.timedelta(seconds=conf.bills_settings.BILL_LIVE_TIME) - datetime.datetime.now()))
    ...
    @property
    def is_percents_barier_not_passed(self): return self.votes_for_percents < conf.bills_settings.MIN_VOTES_PERCENT
    ...
    def get_minimum_created_time_of_active_bills(cls):
        ...
        created_at =  models.Bill.objects.filter(state=relations.BILL_STATE.VOTING).aggregate(Min('created_at'))['created_at__min']
    ...
    def can_vote(self, hero):
        allowed_places_ids = places_logic.get_hero_popularity(hero.id).get_allowed_places_ids(conf.bills_settings.PLACES__TO_ACCESS_VOTING)
    ...
    def apply(self):
        ...
        self._model.min_votes_percents_required = conf.bills_settings.MIN_VOTES_PERCENT
        ...
        with django_transaction.atomic():
            ...
            with achievements_storage.achievements_storage.verify(type=achievements_relations.ACHIEVEMENT_TYPE.POLITICS_ACCEPTED_BILLS, object=self.owner):
                self.save()
...
''' % {'text': text,
    ...
    def create(cls, owner, caption, bill, chronicle_on_accepted, depends_on_id=None):
        ...
        thread = forum_prototypes.ThreadPrototype.create(forum_prototypes.SubCategoryPrototype.get_by_uid(conf.bills_settings.FORUM_CATEGORY_UID),
                                        caption=caption,
                                        author=accounts_logic.get_system_user(),
                                        text=bill_prototype.bill_info_text(text),
                                        technical=True,
                                        markup_method=forum_models.MARKUP_METHOD.POSTMARKUP)
    ...
    def get_applicable_bills_ids(cls):
        time_barrier = datetime.datetime.now() - datetime.timedelta(seconds=conf.bills_settings.BILL_LIVE_TIME)
...
class VotePrototype(utils_prototypes.BasePrototype):
    ...
    def create(cls, owner, bill, type): # pylint: disable=W0622
        ...
        with achievements_storage.achievements_storage.verify(type=achievements_relations.ACHIEVEMENT_TYPE.POLITICS_VOTES_AGAINST, object=owner):
            with achievements_storage.achievements_storage.verify(type=achievements_relations.ACHIEVEMENT_TYPE.POLITICS_VOTES_FOR, object=owner):
                with achievements_storage.achievements_storage.verify(type=achievements_relations.ACHIEVEMENT_TYPE.POLITICS_VOTES_TOTAL, object=owner):
