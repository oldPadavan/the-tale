...
class BillResource(utils_resources.Resource):
    ...
    def index(self, page=1, owner=None, state=None, bill_type=None, voted=None, place=None):#pylint: disable=R0914
        ...
        paginator = utils_pagination.Paginator(page, bills_count, conf.bills_settings.BILLS_ON_PAGE, url_builder)
    ...
    def create(self, bill_type):
        ...
        if datetime.datetime.now() - self.account.created_at < datetime.timedelta(days=conf.bills_settings.MINIMUM_BILL_OWNER_AGE):
            return self.json_error('bills.create.too_young_owner',
                                   'Новые игроки не могут создать запись в течении %d дней с момент регистрации' % conf.bills_settings.MINIMUM_BILL_OWNER_AGE)
